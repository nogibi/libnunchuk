...
NunchukImpl::NunchukImpl(const AppSettings& appsettings,
    ...
    : app_settings_(appsettings),
      ...
      group_service_(app_settings_.get_group_server(),
                     app_settings_.get_certificate_file()) {
...
std::vector<Wallet> NunchukImpl::GetWallets(const std::vector<OrderBy>& orders,
                                            bool skip_balance) {
...
SingleSigner NunchukImpl::ParseKeystoneSigner(const std::string& qr_data) {
  ...
  auto signer = SingleSigner("Keystone", key.get_xpub(), {}, key.get_path(),
                             {0, 1}, account.get_master_fingerprint(), 0);
...
std::vector<SingleSigner> NunchukImpl::ParseSeedSigners(
  ...
  const std::string xfp = account.get_master_fingerprint();
  ...
  for (auto&& key : account.outputDescriptors) {
    ...
    signers.emplace_back(SingleSigner(
        ...
        {}, path, {0, 1}, xfp, 0, {}, false, SignerType::AIRGAP));
  }
...
std::string NunchukImpl::CreatePsbt(
  ...
  bool is_taproot = wallet.get_address_type() == AddressType::TAPROOT;
  ...
  if (has_silent_payment) {
    ...
    for (const auto& input : sp_inputs) {
      ...
      CExtKey key = software_signer.GetExtKeyAtPath(
          ...
          std::to_string(index));
      input_privkeys.push_back(is_taproot ? CalculateTweakedKey(key.key)
                                          : key.key);
    }
